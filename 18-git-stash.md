...
1. **Modifier `frontend.js`** :
   ...
   // Fonction temporaire pour tests
   function debugMode() {
       console.log('Mode debug activé');
   }" >> frontend.js
   ```

2. **Modifier `backend.py`** :

   ```bash
   echo "
   # Nouvelle fonctionnalité en développement
   def handle_request(request_data):
       print('Traitement requête:', request_data)
       return {'status': 'processing', 'data': request_data}

   # Code expérimental
   def experimental_feature():
       return 'Fonctionnalité en test'" >> backend.py
   ```
...
3. **Lister tous les stashes** :
   ...
   stash@{1}: WIP on main: Ajout fonctionnalités frontend et backend
   ```
